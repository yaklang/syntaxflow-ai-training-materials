...
# 用法：在 syntaxflow-ai-training-materials 根目录执行 ./scripts/build-syntaxflow-aikb-to-libs.sh
...
# 知识库根目录：脚本在 scripts/ 下，父目录即 knowledge base 根
SF_BASE="."
if [[ ! -d "basic-syntax" ]] || [[ ! -d "practice" ]]; then
  echo "Error: basic-syntax or practice not found, run from syntaxflow-ai-training-materials root" >&2
  exit 1
fi
